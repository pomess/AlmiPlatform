...
from atlas_runtime.config import repo_root
...
def _resolve_path() -> Path:
    """Return the audit log path, honoring ``DISEASE360_AUDIT_PATH`` if set."""
    override = config_get("DISEASE360_AUDIT_PATH")
    if override:
...
def _path() -> Path:
    global _PATH
    # Resolve lazily; tests use a per-test DISEASE360_HOME and reset module state,
