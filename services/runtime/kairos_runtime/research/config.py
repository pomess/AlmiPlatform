...
All values overridable via env vars loaded through kairos_runtime.config.
"""
...
def get_model(role: str) -> str:
    ...
    env_key = f"KAIROS_RESEARCH_{role.upper()}_MODEL"
...
def get_token_budget(tier: DepthTier) -> int:
    env_key = f"KAIROS_RESEARCH_BUDGET_{tier.upper()}"
...
def get_timeout(tier: DepthTier) -> int:
    env_key = f"KAIROS_RESEARCH_TIMEOUT_{tier.upper()}"
    override = get(env_key)
    if override:
        return int(override)
...
def resolve_tier(tier: DepthTier) -> TierConfig:
    max_conc = int(get("KAIROS_RESEARCH_MAX_CONCURRENT") or MAX_CONCURRENT_SUBAGENTS)
    max_rnd = int(get("KAIROS_RESEARCH_MAX_ROUNDS") or MAX_ROUNDS)
    max_tc = int(get("KAIROS_RESEARCH_MAX_TOOL_CALLS") or MAX_TOOL_CALLS_PER_SUBAGENT)
    return TierConfig(
        tier=tier,
