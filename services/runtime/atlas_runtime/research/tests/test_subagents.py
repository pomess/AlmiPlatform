...
from atlas_runtime.research.subagents import _make_fetch_tool, _make_search_tool
...
async def test_search_semaphore_caps_concurrent_calls():
    ...
    with patch(
        "atlas_runtime.research.grounding.grounded_search",
...
async def test_fetch_semaphore_caps_concurrent_calls():
    ...
    with patch("atlas_runtime.research.fetch.fetch_url", side_effect=_fake_fetch):
...
async def test_fetch_tool_short_circuits_when_over_budget():
    ...
    with patch("atlas_runtime.research.fetch.fetch_url", new=fetch_mock):
