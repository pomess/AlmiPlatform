...
from disease360_runtime.research.state import (
    Classification,
    Domain,
    QueryType,
    ResearchBrief,
...
async def test_deep_research_end_to_end(mock_classification, mock_brief):
    ...
    with (
        patch(
            "disease360_runtime.research.runner.classify_and_brief",
            AsyncMock(return_value=(mock_classification, mock_brief)),
        ),
        patch(
            "disease360_runtime.research.runner.build_lead_agent",
            return_value=mock_agent,
        ),
        patch(
            "disease360_runtime.research.runner.verify_findings",
            AsyncMock(return_value=_mock_verify()),
        ),
        patch(
            "disease360_runtime.research.runner.synthesize_report",
...
async def test_deep_research_handles_lead_timeout(mock_classification, mock_brief):
    ...
    with (
        ...
        patch(
            "disease360_runtime.research.config.WALL_CLOCK_TIMEOUT_SECONDS",
...
async def test_subagent_tokens_recorded_in_meter(mock_classification, mock_brief):
    ...
    """
    from disease360_runtime.research.runner import TokenMeter
