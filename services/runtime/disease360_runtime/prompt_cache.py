...
2. **Explicit caching** via `client.caches.create(...)` (this module).
   ...
   `DISEASE360_USE_CONTEXT_CACHE` config flag (default OFF). Enable only on
...
def context_cache_enabled() -> bool:
    """Truthy iff `DISEASE360_USE_CONTEXT_CACHE` is set to a truthy value."""
    raw = (config.get("DISEASE360_USE_CONTEXT_CACHE") or "").strip().lower()
