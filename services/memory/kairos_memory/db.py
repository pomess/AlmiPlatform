...
``KAIROS_VAULT_BACKEND`` gates whether the disk or Postgres code path is
...
def vault_backend() -> str:
    ...
    """
    val = (config_get("KAIROS_VAULT_BACKEND", DEFAULT_BACKEND) or DEFAULT_BACKEND).strip().lower()
...
def dev_tenant_id() -> str:
    ...
    Required when ``KAIROS_VAULT_BACKEND=postgres`` and there's no JWT in
    flight (phase 4 work). Raises if unset to surface misconfiguration
    early instead of silently inserting under a NULL tenant.
    """
    val = config_get("KAIROS_DEV_TENANT_ID", "")
    if not val:
        raise RuntimeError(
            "KAIROS_DEV_TENANT_ID is unset; set it to Bruno's auth.users.id "
...
async def get_pool() -> asyncpg.Pool:
    ...
    async with _pool_lock:
        if _pool is not None:
            return _pool
        dsn = config_get("KAIROS_SUPABASE_DB_URL", "")
        if not dsn:
            raise RuntimeError(
                "KAIROS_SUPABASE_DB_URL is unset; cannot reach Postgres. "
                "Either flip KAIROS_VAULT_BACKEND=disk or set the DSN."
            )
...
def env_snapshot() -> dict[str, Any]:
    ...
    return {
        "KAIROS_VAULT_BACKEND": vault_backend(),
        "KAIROS_SUPABASE_DB_URL_set": bool(os.environ.get("KAIROS_SUPABASE_DB_URL")),
        "KAIROS_SUPABASE_SERVICE_ROLE_KEY_set": bool(
            os.environ.get("KAIROS_SUPABASE_SERVICE_ROLE_KEY")
        ),
        "KAIROS_DEV_TENANT_ID_set": bool(os.environ.get("KAIROS_DEV_TENANT_ID")),
    }
