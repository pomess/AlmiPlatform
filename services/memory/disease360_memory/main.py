...
Run with: `uvicorn disease360_memory.main:app --port 8001`
...
`DISEASE360_VAULT_BACKEND` selects the storage substrate:
...
app = FastAPI(title="Disease360 Memory", version="0.3.0", lifespan=_lifespan)
...
def run() -> None:
    ...
    uvicorn.run("disease360_memory.main:app", host="127.0.0.1", port=8001, reload=False)
